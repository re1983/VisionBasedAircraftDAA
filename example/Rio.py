...
p4 = win.addPlot(title="airspeed", row=3, col=0)
...
def monitor():
    ...
    with xpc.XPlaneConnect() as client:
        ...
        posi = [-23.047998, -43.162848, 1000, 0,    0,   0,  1]
        client.sendPOSI(posi)
        ctrl = [0.0, 0.0, 0.0, 0.6]
        ...
        while True:
            if (datetime.now() > last_update + timedelta(milliseconds = update_interval * 1000)):
                ...
                pitch_PID.SetPoint = normalize(altitude_PID.output, min=-15, max=15)
                ...
                if(len(x_axis_counters) > plot_array_max_length):
                    ...
                    airspeed_histroy.pop(0)
                    # airspeed_setpoint_history.pop(0)
                    ...
                    airspeed_histroy.append(current_asi)
                    # airspeed_setpoint_history.append(100)
                ...
                p4.plot(x_axis_counters, airspeed_histroy, pen=0,clear=True)
                # p4.plot(x_axis_counters, altitude_setpoint_history, pen=1)
